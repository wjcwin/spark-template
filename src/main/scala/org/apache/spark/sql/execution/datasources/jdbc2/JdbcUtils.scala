...
object JdbcUtils extends Logging {
      ...
      case JDBCSaveMode.Update =>
        ...
        val sql =
          if (driver.contains("postgresql")) {
            require(props.containsKey("duplicateKey") && props.getProperty("duplicateKey").nonEmpty, "当upsert postgresql表时，需指定唯一键，参数名：duplicateKey，字段名逗号分割")
            require(props.containsKey("updateFields") && props.getProperty("updateFields").nonEmpty, "当upsert postgresql表时，需指定唯一键，参数名：updateFields，字段名逗号分割")
            val duplicateKey = props.getProperty("duplicateKey")
            val duplicateSetting = props.getProperty("updateFields").split(",")
              .map { f => f + "=excluded." + f }
              .mkString(",")
            s"INSERT INTO $table ($columns) VALUES ($placeholders) ON CONFLICT (${duplicateKey}) DO UPDATE SET ${duplicateSetting}"
          } else {
            val duplicateIncs = props
              ...
              .map { x => s"`$x`" }
            val updateFields = if (props.containsKey("updateFields") && props.getProperty("updateFields").nonEmpty) {
              props.getProperty("updateFields").split(",")
            } else {
              rddSchema.fieldNames
            }
            val duplicateSetting = updateFields
              .map { x => dialect.quoteIdentifier(x) }
  ...
  def resultSetToRows(resultSet: ResultSet, schema: StructType): Iterator[Row] = {
    ...
    internalRows.map(encoder.createDeserializer())
  }
  ...
  private[spark] def resultSetToSparkInternalRows(
    ...
    new NextIterator[InternalRow] {
      ...
      private[this] val getters: scala.Array[JDBCValueGetter] = makeGetters(schema)
   ...
   */
  private def makeGetters(schema: StructType): scala.Array[JDBCValueGetter] =
  ...
  private def makeGetter(dt: DataType, metadata: Metadata): JDBCValueGetter = dt match {
    ...
    case ArrayType(et, _) =>
      val elementConversion = et match {
        case TimestampType =>
          (array: Object) =>
            array.asInstanceOf[scala.Array[java.sql.Timestamp]].map { timestamp =>
        ...
        case StringType =>
          (array: Object) =>
            ...
            array.asInstanceOf[scala.Array[java.lang.Object]]
              .map(obj => if (obj == null) null else UTF8String.fromString(obj.toString))

        case DateType =>
          (array: Object) =>
            array.asInstanceOf[scala.Array[java.sql.Date]].map { date =>
              nullSafeConvert(date, DateTimeUtils.fromJavaDate)
            }

        case dt: DecimalType =>
          (array: Object) =>
            array.asInstanceOf[scala.Array[java.math.BigDecimal]].map { decimal =>
        ...
        case _ => (array: Object) => array.asInstanceOf[scala.Array[Any]]
      }
  ...
  private def makeSetter(
    ...
    case BinaryType ⇒
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) ⇒
        stmt.setBytes(pos + 1, row.getAs[scala.Array[Byte]](pos - offset))
  ...
  private def getSetter(fields: scala.Array[StructField], connection: Connection, dialect: JdbcDialect, isUpdateMode: Boolean): scala.Array[JDBCValueSetter] = {
    val setter = fields.map(_.dataType).map(makeSetter(connection, dialect, _))
    if (isUpdateMode) {
      scala.Array.fill(2)(setter).flatten
    } else {
      setter
    }
  }
  ...
  def savePartition(
    ...
    try {
      ...
      val setters: scala.Array[JDBCValueSetter] = getSetter(rddSchema.fields, conn, dialect, isUpdateMode)
  ...
  private def parseUserSpecifiedCreateTableColumnTypes(
    ...
    SchemaUtils.checkColumnNameDuplication(userSchema.map(_.name), nameEquality)
